...
export const test = base.extend<ApiFixtures, ApiWorkerFixtures>({
    authState: [
        async ({}, use) => {
            ...
            const apiAuthUrl = getRequiredEnv("BACKOFFICE_API_AUTH_URL");
            const apiUsername = getRequiredEnv("BACKOFFICE_API_USERNAME");
            const apiPassword = getRequiredEnv("BACKOFFICE_API_PASSWORD");
            const apiBaseUrl = getRequiredEnv("BACKOFFICE_API_BASE_URL");
            const tokenPath = process.env.BACKOFFICE_API_TOKEN_PATH ?? "token";
